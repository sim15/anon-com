...
// takes 4 blocks and outputs 2 compressed blocks
void mmoHash4to2(struct Hash *hash, uint8_t* input, uint8_t* output) {

    uint128_t* outputblocks = (uint128_t*)output;
    uint128_t* inputblocks = (uint128_t*)input;

    uint128_t prevdigest = 0;
    mmoRound(hash->mmoCtx[0], &prevdigest, &inputblocks[0]);
    mmoRound(hash->mmoCtx[0], &prevdigest, &inputblocks[1]);
    mmoRound(hash->mmoCtx[0], &prevdigest, &inputblocks[2]);
    mmoRound(hash->mmoCtx[0], &prevdigest, &inputblocks[3]);
    outputblocks[0] = prevdigest;

    prevdigest = 0;
    mmoRound(hash->mmoCtx[1], &prevdigest, &inputblocks[0]);
    mmoRound(hash->mmoCtx[1], &prevdigest, &inputblocks[1]);
    mmoRound(hash->mmoCtx[1], &prevdigest, &inputblocks[2]);
    mmoRound(hash->mmoCtx[1], &prevdigest, &inputblocks[3]);
    outputblocks[1] = prevdigest; 
}

// Matyas-Meyer-Oseas technique for instantiating a one-way compression function 
// takes 2 blocks and outputs 4 compressed blocks
