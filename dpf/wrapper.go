...
func (vdpf *Vdpf) BatchVerEval(key *DPFKey, indices []uint64) ([]int64, []bool, []byte) {
	...
	res := make([]int64, len(indices)*2) // returned output is uint128_t
	// resBytes := make([]byte, len(indices))
	resShort := make([]int64, len(indices))
	...
	// b := 0
	// for i := 0; i < len(res); i += 2 {
	// 	resBytes[b] = byte(res[i]) // change dpf outputs
	// 	b++
	// }
	// fmt.Println(resBytes)

	bits := make([]bool, len(res))
	for i := 0; i < len(res); i += 2 {
		resShort[i/2] = res[i] // change dpf outputs
		bits[i/2] = (res[i] & 1) == 1
	}

	return resShort, bits, pi // converted to int64 from uint64 for PRG seed
}
...
func (vdpf *Vdpf) FullDomainVerEval(key *DPFKey) ([]int64, []bool, []byte) {
