...
extern void batchEvalVDPF(EVP_CIPHER_CTX *ctx, struct Hash *hash1, struct Hash *hash2, int size, bool b, unsigned char* k, uint64_t *in, uint64_t inl, uint8_t* out, uint8_t*pi);
extern void fullDomainVDPF(EVP_CIPHER_CTX *ctx, struct Hash *hash1, struct Hash *hash2, int size, bool b, unsigned char* k, uint8_t *out, uint8_t* proof);
